 ...
 */
export function intToDecimal(
  intToConvert: BigNumber.Value,
  ...
  if (typeof intToConvert === 'string' && int === 'NaN') {
    throw new Error(
      `intToDecimal: invalid value '${intToConvert}', should be a BigNumber or string-encoded number.`,
    );
  }
  ...
  if (int.includes('.')) {
    throw new Error(
      `intToDecimal: value '${int}' has too many decimal places.`,
    );
  }
  ...
  if (!SANITY_CHECK.exec(int)) {
    throw new Error(
      `intToDecimal: failed sanity check -` +
        ` value '${int}',` +
        ` does not match (^-?[0-9]+$).`,
    );
  }
 ...
 * @throws When decimal or integer representation is invalid.
 */
export function decimalToInt(
  decimalToConvert: BigNumber.Value,
  fractionLength: number,
): string {
  ...
  if (typeof decimalToConvert === 'string' && decimal === 'NaN') {
    throw new Error(
      `decimalToInt: invalid value '${decimalToConvert}', should be a BigNumber or string-encoded number.`,
    );
  }
  ...
  if (!SANITY_CHECK.exec(decimal)) {
    throw new Error(
      `decimalToInt: failed sanity check - value '${decimal}', does not match (^-?[0-9.]+$).`,
    );
  }
  ...
  if (components.length > 2) {
    throw new Error(
      `decimalToInt: failed sanity check - value '${decimal}' has too many decimal points.`,
    );
  }
  ...
  if (fraction.length > fractionLength) {
    throw new Error(
      `decimalToInt: value '${decimal}' has too many decimal places.`,
    );
  }
