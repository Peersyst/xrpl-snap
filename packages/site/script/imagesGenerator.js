...
function addImages(folder) {
  ...
  for (const filename of filenames) {
    ...
    if (stat.isDirectory()) {
      addImages(folder + (filename.endsWith('/') ? filename : `${filename}/`));
    } else if (filename === '.DS_Store' || filename === 'index.ts') {
      fs.unlinkSync(folder + filename);
    } else {
      imgRequires.push(generateExport(filename, folder.replace(imgFolder, '') + (filename.endsWith('/') ? filename.slice(-1) : filename)));
    }
  }
}
