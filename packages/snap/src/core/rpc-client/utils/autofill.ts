/* eslint-disable */
/**
 * THIS CODE HAS BEEN MODIFIED FROM THE WS CLIENT OF THE xrpl.js LIBRARY
 */
import {
  AccountInfoRequest,
 ...
 */
export function validateAccountAddress(tx: Transaction, accountField: string, tagField: string): void {
 ...
 */
export function txNeedsNetworkID(client: RPCClient): boolean {
 ...
 */
export async function setNextValidSequenceNumber(client: RPCClient, tx: Transaction): Promise<void> {
 ...
 */
async function fetchAccountDeleteFee(client: RPCClient): Promise<BigNumber> {
 ...
 */
export async function calculateFeePerTransactionType(client: RPCClient, tx: Transaction, signersCount = 0): Promise<void> {
  ...
  const totalFee = tx.TransactionType === 'AccountDelete' ? baseFee : BigNumber.min(baseFee, maxFeeDrops);
 ...
 */
export async function setLatestValidatedLedgerSequence(client: RPCClient, tx: Transaction): Promise<void> {
 ...
 */
export async function checkAccountDeleteBlockers(client: RPCClient, tx: Transaction): Promise<void> {
