...
/**
 * COPIED FROM xrpl.js
 * It has been copied as a workaround as when we import from xrpl, it throws an InvalidInputError
 */
 ...
 * @category Utilities
 */
function dropsToXrp(dropsToConvert: BigNumber.Value): number {
  /*
  ...
  if (typeof dropsToConvert === 'string' && drops === 'NaN') {
    throw new InvalidInputError(`dropsToXrp: invalid value '${dropsToConvert}', should be a BigNumber or string-encoded number.`);
  }

  // drops are only whole units
  if (drops.includes('.')) {
    throw new InvalidInputError(`dropsToXrp: value '${drops}' has too many decimal places.`);
  }
  ...
  if (!SANITY_CHECK.exec(drops)) {
    ...
    throw new InvalidInputError(`dropsToXrp: failed sanity check -` + ` value '${drops}',` + ` does not match (^-?[0-9]+$).`);
  }
 ...
 * @category Utilities
 */
function xrpToDrops(xrpToConvert: BigNumber.Value): string {
  ...
  if (typeof xrpToConvert === 'string' && xrp === 'NaN') {
    throw new InvalidInputError(`xrpToDrops: invalid value '${xrpToConvert}', should be a BigNumber or string-encoded number.`);
  }
  ...
  if (!SANITY_CHECK.exec(xrp)) {
    throw new InvalidInputError(`xrpToDrops: failed sanity check - value '${xrp}', does not match (^-?[0-9.]+$).`);
  }

  const components = xrp.split('.');
  if (components.length > 2) {
    throw new InvalidInputError(`xrpToDrops: failed sanity check - value '${xrp}' has too many decimal points.`);
  }

  // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
  const fraction = components[1] || '0';
  if (fraction.length > MAX_FRACTION_LENGTH) {
    throw new InvalidInputError(`xrpToDrops: value '${xrp}' has too many decimal places.`);
  }
