...
export const addMissingPropertiesToToken = (token: Erc20Token, balance?: string, usdPrice?: number): Erc20TokenBalance => {
    return {
        ...token,
...
export const isSpecialInputKey = (event: KeyboardEvent<HTMLInputElement>) => {
    return event.key === "Backspace" || event.ctrlKey || event.key === "ArrowRight" || event.key === "ArrowLeft" || event.metaKey;
};
