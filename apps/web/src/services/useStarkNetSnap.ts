...
export const useStarkNetSnap = () => {
    ...
    const getTransactions = async (
        ...
        try {
            ...
            if (onlyFromState) {
                ...
                storedTxns = [...storedTxns, ...filteredTxns].sort((a: Transaction, b: Transaction) => b.timestamp - a.timestamp);
            }
    ...
    const updateTokenBalance = async (tokenAddress: string, accountAddress: string, chainId: string) => {
        ...
        if (foundTokenWithBalance) {
            ...
            const tokenWithBalance: Erc20TokenBalance = addMissingPropertiesToToken(foundTokenWithBalance, tokenBalance, usdPrice);
            dispatch(upsertErc20TokenBalance(tokenWithBalance));
        }
    };
