...
import { Column, Description, IconStyled, Label, Left, LeftIcon, Middle, Right, Status, Wrapper } from "./TransactionListItem.style";
import { getIcon, getTxnDate, getTxnFailureReason, getTxnName, getTxnStatus, getTxnToFromLabel, getTxnValues } from "./types";
...
export const TransactionListItemView = ({ transaction }: Props) => {
    ...
    return (
        <Wrapper onClick={() => openExplorerTab(transaction.txnHash, "tx", transaction.chainId)}>
            ...
            <Right>
                {txnName === "Send" && <AssetQuantity currency={currencySymbol} currencyValue={txnValue} USDValue={txnUsdValue} />}
            </Right>
        </Wrapper>
    );
};
